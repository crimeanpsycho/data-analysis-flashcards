...
# Есть ли связь между gender (M/F) и workout_type (C/S/Y/D)?
# Оба признака КАТЕГОРИАЛЬНЫЕ → хи-квадрат тест
# Шапиро-Уилка НЕ нужен (он только для числовых данных)
# ────────────────────────────────────────────────────────────────
...
# ════════════════════════════════════════════════════════════════
# ДОПОЛНИТЕЛЬНО: Связь gender × workout_type → Хи-квадрат
# ════════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("Хи-квадрат: gender × workout_type")
print("="*60)

from scipy.stats import chi2_contingency

# Шаг 1: Таблица сопряжённости (contingency table)
contingency = pd.crosstab(df_clean['gender'], df_clean['workout_type'])
print("\nТаблица сопряжённости:")
print(contingency)

# Шаг 2: Хи-квадрат тест
chi2, p_chi, dof, expected = chi2_contingency(contingency)
print(f"\nХи-квадрат статистика : {round(chi2, 2)}")
...
alpha = 0.05
if p_chi < alpha:
    print(f"\np={p_chi:.4f} < {alpha} → H0 ОТВЕРГАЕТСЯ")
    print("Вывод: статистически значимая связь между полом и типом тренировки ЕСТЬ")
else:
    print(f"\np={p_chi:.4f} >= {alpha} → H0 НЕ отвергается")
    print("Вывод: статистически значимой связи между полом и типом тренировки НЕТ")
